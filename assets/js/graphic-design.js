(() => {
  ...
  const openViewer = (item, selectedView = 0) => {
    ...
    order.forEach((work, workIndex) => {
      ...
      const render = () => { const media = work.media[viewIndex]; const element = mediaElement(media, titleFor(work)); screenWhiteBackground(element); zoom.render(element); if (media.type === 'image') { const candidate = highResolutionSource(media); const probe = new Image(); probe.addEventListener('load', () => zoom.setZoomSource(candidate), { once: true }); probe.src = candidate; } };
      render();
  ...
  const cardFor = (item) => { const details = metadataFor(item); const fragment = template.content.cloneNode(true); const card = fragment.querySelector('.painting-card'); const media = fragment.querySelector('.painting-card__media'); const preview = fragment.querySelector('.painting-card__preview'); const navigation = fragment.querySelector('.painting-card__navigation'); let viewIndex = 0; const render = () => { preview.replaceChildren(); const element = mediaElement(item.media[viewIndex], titleFor(item)); if (item.title === 'Jacked Up Shirt' && element.tagName === 'IMG') element.src = 'assets/img/Graphic Design Thumbnails/Jacked Up Shirt.png'; if (element.tagName === 'VIDEO') { element.controls = false; element.autoplay = true; element.loop = true; } preview.append(element); screenWhiteBackground(element); navigation.querySelectorAll('.painting-card__dot').forEach((dot, index) => { if (index === viewIndex) dot.setAttribute('aria-current', 'true'); else dot.removeAttribute('aria-current'); }); }; render(); media.addEventListener('click', (event) => { if (!event.target.closest('button')) openViewer(item, viewIndex); }); if (item.media.length > 1) { const addArrow = (direction, label) => { const button = document.createElement('button'); button.className = `painting-card__${direction}`; button.type = 'button'; button.setAttribute('aria-label', `${label} view of ${titleFor(item)}`); const icon = document.createElement('img'); icon.src = `assets/img/${direction === 'previous' ? 'leftarrow' : 'rightarrow'}.png`; icon.alt = ''; button.append(icon); button.addEventListener('click', () => { viewIndex = (viewIndex + (direction === 'next' ? 1 : -1) + item.media.length) % item.media.length; render(); }); navigation.append(button); }; addArrow('previous', 'Previous'); const dots = document.createElement('div'); dots.className = 'painting-card__dots'; item.media.forEach((_, index) => { const dot = document.createElement('button'); dot.className = 'painting-card__dot'; dot.type = 'button'; dot.setAttribute('aria-label', `View ${index + 1} of ${item.media.length} for ${titleFor(item)}`); if (index === viewIndex) dot.setAttribute('aria-current', 'true'); dot.addEventListener('click', () => { viewIndex = index; render(); }); dots.append(dot); }); navigation.append(dots); addArrow('next', 'Next'); } fragment.querySelector('.painting-card__title').textContent = titleFor(item); fragment.querySelector('.painting-card__description').textContent = details.description; [clientFor(item), ...details.types].forEach((tag) => { const li = document.createElement('li'); const button = document.createElement('button'); button.type = 'button'; button.textContent = tag; button.addEventListener('click', () => selectTag(tag)); li.append(button); fragment.querySelector('.painting-card__tags').append(li); }); card.dataset.graphicItem = item.id; return { fragment, card }; };
